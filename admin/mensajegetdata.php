<?php 
	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
	        echo "
		        <tr data-value='".$row['intidmensaje']."'>
		        	<td>CAT".$row['intidmensaje']."</td>
		        	<td>".$row['nvchmensaje']."</td>
		        	<td>
		        		<a href='mensaje_editar.php?codigo=".$row['intidmensaje']."' class='btn btn-info btn-xs'>Editar</a>
		        		<a href='#' data-value='".$row['intidmensaje']."' onclick='' id='btn-eliminar' class='btn btn-danger btn-xs' data-toggle='modal' data-target='#myModal'>Eliminar</a>
		        	</td>
		        </tr>
	        ";
	    }
