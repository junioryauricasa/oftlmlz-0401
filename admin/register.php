<?php
	...
	if (isset($_POST['signup'])) {
		...
		if (!preg_match("/^[a-zA-Z ]+$/",$name)) {
			$error = true;
			$name_error = "<b>El nombre debe contener solo caracteres del alfabeto y espacio.</b>";
		}
		if(!filter_var($email,FILTER_VALIDATE_EMAIL)) {
			$error = true;
			$email_error = "<b>Ingresa un correo electrónico válido.</b>";
		}
		if(strlen($password) < 6) {
			$error = true;
			$password_error = "<b>La contraseña debe tener un mínimo de 6 caracteres.</b>";
		}
		if($password != $cpassword) {
			$error = true;
			$cpassword_error = "<b>Las contraseñas no coinciden</b>";
		}
		if(!$terminosycond) {
			//$error = true;
...
<div class="container">
			...
			<form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="signupform">
				<fieldset>
					...
					<div class="form-group">
						...
						<!-- Texto de validacion de campo -->
						<span class="text-danger"><?php if (isset($password_error)) echo $password_error; ?></span>
						<br>
						<!-- Ver Contraseña -->
						<!--a href="#" class="btn btn-xs btn-info">Ver Contraseña</a-->
						<input id="show_password" type="checkbox" /> Mostrar contraseña
						<br>
						<!-- btn generar contraseña -->
						<a href="#" onclick="generar()" class="btn btn-xs btn-danger">Generar Contraseña Segura</a>

					</div>

					<div class="form-group">
						<label for="name">Repita Contraseña</label>
						<input type="password"  id="cpassword" name="cpassword" placeholder="Confirmar Contraseña" required class="form-control" />
	...
	<div class="row">
		...
		Ya te registaste? <a href="login.php" class="btn btn-default btn-xs">Logeate Aqui</a>
		</div>
	</div>
</div>
...
<script>
	...
	function generar()
	{
	  ...
	  long=18; // Longitud de la contraseña
	  var caracteres = "abcdefghijkmnpqrtuvwxyzABCDEFGHIJKLMNPQRTUVWXYZ2346789@/-&%$#()=/?¡¿°!Ø${├ ⭐❤✅️⛔️✌️⚡️";
	  var contraseña = "";
	  ...
	  document.getElementById("cpassword").value=contraseña;

	  //document.getElementById(“txtTabla”).value = “Tutorial Javascript”;
	}

	/*
	Funcion: Mostrar / Ocultar contraseña mediante un checkbox. 
